...
public class ExampleWBS extends Application {
	...
	static Component accumulatorPump = new Pump("Accumulator Pump", 0, 1);
	...
	static Component pipeB2 = new Pipe("Pipe B2", 2, 1);
	static Component pipeG3 = new Pipe("Pipe G3", 1, 0);
	static Component pipeB3 = new Pipe("Pipe B3", 1, 0);

	static Component selectorValve = new Valve("Selector Valve", 3, 2);
	static Component meterValveGreen = new Valve("Meter Valve green", 2, 1);
	static Component meterValveBlue = new Valve("Meter Valve blue", 2, 1);

	static Component bscu = new BSCU("BSCU", 0, 3);
	static Component[] systemComponents = { greenPump, bluePump, accumulatorPump, pipeG1, pipeG2, pipeG3, pipeB1,
			pipeB2, pipeB3, selectorValve, meterValveBlue, meterValveGreen, bscu };
...
//							theList.add(newList);
	...
	public static boolean systemHasFailed() {
		if (!WBS)
			return systemComponents[0].hasFailed();
		return pipeG3.hasFailed() && pipeB3.hasFailed();
	}
	...
	public void start(Stage stage) throws IOException {
		...
		selectorValve.inputPorts[0] = pipeG1;
		pipeG1.outputPorts[0] = selectorValve;

		selectorValve.inputPorts[1] = pipeB1;
		pipeB1.outputPorts[0] = selectorValve;

		selectorValve.inputPorts[2] = bscu;
		bscu.outputPorts[0] = selectorValve;

		selectorValve.outputPorts[0] = pipeG2;
		pipeG2.inputPorts[0] = selectorValve;

		selectorValve.outputPorts[1] = pipeB2;
		pipeB2.inputPorts[0] = selectorValve;

		pipeG2.outputPorts[0] = meterValveGreen;
		meterValveGreen.inputPorts[0] = pipeG2;

		pipeB2.outputPorts[0] = meterValveBlue;
		meterValveBlue.inputPorts[0] = pipeB2;

		meterValveGreen.outputPorts[0] = pipeG3;
		pipeG3.inputPorts[0] = meterValveGreen;

		meterValveGreen.inputPorts[1] = bscu;
		bscu.outputPorts[1] = meterValveGreen;

		meterValveBlue.outputPorts[0] = pipeB3;
		pipeB3.inputPorts[0] = meterValveBlue;

		meterValveBlue.inputPorts[1] = bscu;
		bscu.outputPorts[2] = meterValveBlue;

		accumulatorPump.outputPorts[0] = pipeB2;
		pipeB2.inputPorts[1] = accumulatorPump;
		
		pipeG3.isTopLevelComponent = true;
		pipeB3.isTopLevelComponent = true;
		...
		fxmlLoader.setLocation(getClass().getResource("Views/WBSindex.fxml"));
