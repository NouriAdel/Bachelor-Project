...
import components.Engine.EngineStates;
...
public class ExampleSystem2 extends Application {
	...
	static FailureType[] possibleCylinderFailures = { FailureType.STUCK, FailureType.LEAK, FailureType.DISCONNECTED };
...
//			FailureType.OVER_VOLTAGE, FailureType.UNDER_VOLTAGE, 
	...
	static FailureType[] possiblePFCFailures = { 
			FailureType.LOSS_OR_INVALID_CONTROL_SIGNAL,
			FailureType.LOSS_OF_INPUT_SIGNAL,
	};
	static FailureType[] possibleACEFailures = { 
			FailureType.LOSS_OR_INVALID_CONTROL_SIGNAL,
			FailureType.LOSS_OF_INPUT_SIGNAL,
	};
	static FailureType[] possibleDELFailures = { 
			FailureType.LOSS_OF_ELECTRICAL_SIGNAL
	};
	static FailureType[] possiblePowerSupplyFailures = { 
			FailureType.LOSS_OF_POWER_SUPPLY
	};
	static FailureType[] possibleADIRUFailures = { 
			FailureType.LOSS_OR_INVALID_OUTPUT_SIGNAL, 
			FailureType.LOSS_OF_POWER_SUPPLY
	};
	
	static FailureType[] possibleControlRodFailures = { 
			FailureType.DISCONNECTED,
			FailureType.JAMMING,
	};
	
	static FailureType[] possibleEngineFailures = { 
			FailureType.SURGE,
			FailureType.STALL,
			FailureType.FLAME_OUT, 
			FailureType.OVER_SPEED, 
			FailureType.OVER_TEMPERATURE,
			FailureType.FUEL_SUPPLY_FAILURE,
	};
	...
	public static ArrayList<TreeEvent> testCheckBeforeInjecting(Component[] systemComps) {
		...
		for (Component component : systemComps) {
			addComponentNewListsWithAllFCs(theList, new ArrayList<FailureCombination>(), component, eventAcc);
		}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					for (Component component : list) {
						...
						for (Component port : inputPorts) {
							...
							addComponentNewListsWithAllFCs(theList, toBeReplacedList, port, eventAcc);
						}
					}
...
//
//							theList.add(newList);
							addComponentNewListsWithAllFCs(theList, FCList, absorber, eventAcc);
							...
							for (Component absorberPort : absorberPorts) {
								if (!absorberPort.hasFailed()) {
									addComponentNewListsWithAllFCs(theList, FCList, absorberPort, eventAcc);
								}
...
//			System.out.println(theList);
	...
	public static ArrayList<FailureCombination> getAllComponentFailureCombinations(Component component) {
		...
		case ACTUATOR_CONTROL_ELECTRONICS:
			for (FailureType failureType : possibleACEFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
		case DIRECT_ELECTRICAL_LINK:
			for (FailureType failureType : possibleDELFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
		case PRIMARY_FLIGHT_CONTROL:
			for (FailureType failureType : possiblePFCFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
		case POWER_SUPPLY:
			for (FailureType failureType : possiblePowerSupplyFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
		case AIR_DATA_AND_INERTIAL_REFERENCE_UNIT:
			for (FailureType failureType : possibleADIRUFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
		...
		case BSCU:
			...
			break;	
		case ENGINE:
			for (FailureType failureType : possibleEngineFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;
		case CONTROL_ROD:
			for (FailureType failureType : possibleControlRodFailures) {
				FailureCombination failureCombination = new FailureCombination(component, failureType);
				componentFCs.add(failureCombination);
			}
			break;	
	...
	public static void addComponentNewListsWithAllFCs(ArrayList<ArrayList<FailureCombination>> theList, 
			ArrayList<FailureCombination> toBeReplacedFCList, Component component, ArrayList<TreeEvent> eventAcc) {
		...
		for (FailureCombination newFC : newComponentFCsList) {
			...
			for (FailureCombination FC : newComponentFCsList) {
			...
			// Test if we this combination has any subsets in the event list
			TreeEvent potentialEvent = createEvent(list, failures);
			boolean covered = eventListCoversEvent(potentialEvent, eventAcc);
