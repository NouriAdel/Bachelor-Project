...
public class GUIControllerExampleFiles {
	@FXML
	private HBox hbox;
...
//	private SystemController systemViewController;
	...
	public GUIControllerExampleFiles(ArrayList<TreeEvent> list, String topEventText) {
		this.list = list;
		this.topEventLabel = topEventText;

		secondaryLabels = new Label[10000];
		itr = 0;

//		systemViewController = new SystemController();
	}
	...
	public void setButtonsActions() {
		...
		homeViewController.modifyCompBtn.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent arg0) {
				...
				ComponentLabel tobeModifiedComponent = ((Component) homeViewController.modifyCompCombobox
						.getValue()).label;
...
//				drawConnections();
	...
	public void initialize() {
		...
		secondaryLabelList = new ArrayList<Label>();

		eventListLabelList = new ArrayList<Label>();
		eventListSecondaryLabelList = new ArrayList<Label>();
		...
		secondaryEventsHbox.setStyle("-fx-spacing: 10;");
			...
			if (eventSize > 1) {
				...
				for (int i = 0; i < eventSize; i++) {
					secondarylabel = new Label(
							"loss of: " + event.componentNames[i] + " due to: " + event.componentFailures[i]);
		...
		double secondaryEventsHBoxSpaing = 10;
		double operatorWidth =  ((Label) secondaryOperatorsHbox.getChildren().get(0)).getPrefWidth();
		int numberOfSecondaryEvents = secondaryEventsHbox.getChildren().size();
		int numberOfSecondaryOperators =  secondaryOperatorsHbox.getChildren().size();
		double widthPerLabel = ((Label) secondaryEventsHbox.getChildren().get(0)).getPrefWidth(); 
		double labelsOverallSpace = (widthPerLabel + secondaryEventsHBoxSpaing) * numberOfSecondaryEvents;
//		System.out.println("Width per label" + widthPerLabel);
//		System.out.println("Spacing per label" + secondaryEventsHBoxSpaing);
		double spacing = labelsOverallSpace / numberOfSecondaryOperators - operatorWidth;
//		System.out.println(labelsOverallSpace);
//		System.out.println("SPACING" + spacing);
		secondaryOperatorsHbox.setStyle("-fx-spacing: " + spacing + ";");
		...
		Label topEventlabel = new Label(this.topEventLabel);
...
//	}
	...
	public void customizeLabel(Label label) {
		label.setWrapText(true);
		label.setStyle("-fx-background-color: #effcfa; -fx-padding: 5px; -fx-text-wrap: true;");
		label.setPrefSize(70, 200);
		label.setTextAlignment(TextAlignment.CENTER);
	}
